...
def ImageJ_processing(folderpath: str,
    ...
    def process_image(i: int, 
        ...
        norm_arr = exposure.rescale_intensity(arr, out_range=(0, 255))
        norm_arr_uint8 = (norm_arr + 0.5).astype(np.uint8) #the +0.5 is so that it rounds to the nearest integer instead of always down
        x_array = xarray.DataArray(norm_arr_uint8, name='imp', dims=('pln','row','col'))
    ...
    with tqdm(total=numfiles, desc='Processing Images', unit='img') as pbar:
        for i in range(numfiles):
            ...
            process_image(i, filepath, filename)
